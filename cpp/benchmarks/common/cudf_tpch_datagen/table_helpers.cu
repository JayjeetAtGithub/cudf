...
cudf::size_type calc_l_cardinality(cudf::column_view const& o_orderkey_repeat_freqs,
                                   rmm::cuda_stream_view stream,
                                   rmm::device_async_resource_ref mr)
{
  CUDF_FUNC_RANGE();
...
std::unique_ptr<cudf::column> calc_charge(cudf::column_view const& extendedprice,
